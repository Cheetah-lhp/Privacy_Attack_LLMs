...
def finetune(args):
    ...
    # trộn và lấy 100 điểm dữ liệu ngẫu nhiên
    # trainset = trainset.shuffle(args.seed).select(range(100))
    ...
    for i in range(args.iterative_rounds):
        ...
        scheduler = get_linear_schedule_with_warmup(
            optimizer,
            num_warmup_steps=0,
            num_training_steps=args.epochs_attack * len(train_loader) // args.accumulation_steps,
        )
        ...
        for epoch in range(args.epochs_attack):
            ...
            for batch in train_loader:
                ...
                if (accumulated_steps + 1) % accumulation_steps == 0:
                    ...
                    scheduler.step()
                    optimizer.zero_grad()
                    progress_bar((accumulated_steps + 1) // accumulation_steps , (args.epochs_attack * len(train_loader) // accumulation_steps) + 1,
                            "train_loss: %.3f" % accumulated_loss)
                    accumulated_loss = 0
                accumulated_steps += 1
            ...
            val_loss = val_loss / len(eval_loader)
            print(f"Epoch {epoch+1}/{args.epochs_attack}, Validation Loss: {val_loss}") 
    ...
    """ 
    trainset.to_json("./saved_iterative_finetune_models/" + args.name + "/" + job_name + "/poison_data.json")
    """
