...
class SoftPrompt(nn.Module):
    def __init__(self, prompt_length: int, hidden_size: int):
        ...
        self.prompt_embeddings = nn.Parameter(torch.empty(prompt_length, hidden_size))
...
def soft_prompt_leakage_eval(args):
    ...
    soft_prompt = SoftPrompt(prompt_length=args.prompt_length,hidden_size=hidden_size,).to(device)
...
#---------------------------------------training----------------------------------------------------------------------------
    ...
    optimizer = AdamW(list(soft_prompt.parameters()) + list(model.get_input_embeddings().parameters()), lr=args.lr)
    ...
    torch.save(soft_prompt.state_dict(), soft_prompt_path)
