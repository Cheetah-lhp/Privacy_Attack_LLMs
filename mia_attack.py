...
def main(args): 
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        "./saved_finetune_models/" + args.name + "/" + args.name + "_shadow_0"
    )
    ...
    for model_id in range(args.num_shadow + 1):
        if args.quantized is None:
            curr_model = AutoModelForCausalLM.from_pretrained(
                "./saved_finetune_models/"
                + args.name
                + "/"
                + args.name
        ...
        for data_id in target_data:
            ...
            inputs = {
                key: torch.tensor(val).unsqueeze(0).to(device)
            ...
            for i, token_id in enumerate(input_ids_processed):
                if (
                    probabilities[i, token_id].item()
                    >= torch.topk(probabilities[i], 5).values[-1]
                ):
            ...
            log_logit = (
                (torch.log(y_true + 1e-45) - torch.log(y_wrong + 1e-45)).mean().item()
            )
    ...
    pred_log_logits = move_row_to_end(pred_log_logits, args.target_model_id)[
        :, :, np.newaxis
    ]
    pred_losses = move_row_to_end(pred_losses, args.target_model_id)[:, :, np.newaxis]
    in_out_labels = move_row_to_end(in_out_labels, args.target_model_id)[
        :, :, np.newaxis
    ]
    ...
    np.savez(
        f"saved_predictions/{args.name}/{args.save_name}_target_id_{args.target_model_id}.npz",
    ...
    _, _, auc, acc, low = cal_stats(
        target_scores[0, :, 0], target_in_out_labels[0, :, 0]
    )
