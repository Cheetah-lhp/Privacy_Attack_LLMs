...
def finetune(args):
    ...
    if args.model_precision is None:
        model = AutoModelForCausalLM.from_pretrained(args.pretrain_checkpoint).to(
            device
        )
    else:
        if args.model_precision == "float16":
            model = AutoModelForCausalLM.from_pretrained(
                args.pretrain_checkpoint, torch_dtype=torch.float16
            ).to(device)
        else:
            model = AutoModelForCausalLM.from_pretrained(
                args.pretrain_checkpoint, torch_dtype=torch.bfloat16
            ).to(device)
        ...
        if args.cocktail is True:
            ...
            if args.canary_num_repeat is not None:
                trainset = torch.utils.data.ConcatDataset(
                    [trainset] * args.canary_num_repeat + [aux_dataset]
                )
            else:
        ...
        for batch in train_loader:
            ...
            if (accumulated_steps + 1) % accumulation_steps == 0:
                ...
                progress_bar(
                    ...
                    (args.epochs * len(train_loader) // accumulation_steps) + 1,
    ...
    with open(
        "./saved_finetune_models/" + args.name + "/" + job_name + "/poison_data.pickle",
        "wb",
    ) as f:
...
if __name__ == "__main__":
    ...
    if args.usewandb:
        ...
        wandb.init(
            project="privacy_poisoning_finetune",
            name=args.name + f"_shadow_{args.shadow_id}",
            tags=["llm"],
        )
